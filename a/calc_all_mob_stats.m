...
group_set = {'0' '1' '2' '3' '4'};
output_folder = '~/iec/mobility/bounds';
moment_folder = '~/iec/mobility/moments';
...
for g = 1:length(group_set)
    ...
    for i = 1:length(bc_set)
        input_csv = sprintf('%s/ed_ranks_%s_%d.csv', moment_folder, group, bc_set(i));
        fprintf('250...');
    ...
    % set target filename
    output_csv_coef = sprintf('%s/coef_%s.csv', output_folder, group);
    output_csv_p25 = sprintf('%s/p25_%s.csv', output_folder, group);
    output_csv_mu50 = sprintf('%s/mu50_%s.csv', output_folder, group);
